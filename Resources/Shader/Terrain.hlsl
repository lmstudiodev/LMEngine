...
VPS_INOUT vsmain(VS_INPUT input)
{
    ...
    float texelSize = 1.0f / m_terrainData.heightMapSize;
    
    float height = HeightMap.SampleLevel(HeightMapSampler, input.texcoord, 0).r;
    output.position = mul(float4(input.position.x * m_terrainData.size.x, height * m_terrainData.size.y, input.position.z * m_terrainData.size.z, 1), m_worldMatrix);
...
{
    float4 color = float4(1,1,1,1); //Color.Sample(ColorSampler, input.texcoord);
